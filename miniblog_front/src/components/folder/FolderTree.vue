<template>
    <div class="folder-tree" v-bind:style="{color:myColor}" v-if="folder.isShow">
        <div class="folder-pen">
            ...
            <div class="pack-up" @click="folder.isStretch = !folder.isStretch">
                <i class="fa" :class="[{'fa-caret-left': !folder.isStretch },{'fa-caret-down':folder.isStretch}]"></i>
            </div>
        </div>
        <folder-tree-contents v-if="!folder.card && folder.isStretch"
        ...
        methods: {
            selectedFolder() {
                if (this.selectedState.folder.name && this.folder.id === this.selectedState.folder.id) {
                    ...
                    this.selectedState.folder.rootId = this.rootId;
                }
